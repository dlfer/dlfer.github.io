 ...
 for s in range(maxs):
  ...
  while not is_lone(M,s):
   ...
   if is_lone(M,s):
    ...
    if res:
     # if it does  not divide all, add the proper row and it is not lone any longer...
     x,y=res
     add_to_row(M,s,1,x)
     add_to_row(L,s,1,x)
    else:
...
M=matrix(ZZ,CK.differential(1)).numpy().tolist()

Smith(M)
print "\nsmith form of d_1:\n", display(M)

M=matrix(ZZ,CK.differential(2)).numpy().tolist()
Smith(M)
print "\nsmith form of d_2:\n", display(M)
